...
def read_dictionaries(rarefied_counts):
    arg_index = {}
    with open('../arg_index.txt', 'r') as f:
        for line in f:
            items = line.split('\t')
            arg_index[items[0]] = items[1]

    established_index = {}
    with open('../blastout_established.tsv', 'r') as f:
        for line in f:
            items = line.split('\t')
...
def main():
    ...
    arg_index, established_index, taxonomy_map, species_index = read_dictionaries(counts)
    ...
    counts.loc['taxonomy_score'] = [get_taxonomy_score(species_index[arg], taxonomy_map) for arg in tqdm(counts.columns)]
    counts.loc['n_pathogens'] = [count_pathogens(species_index[arg], pathogen_index) for arg in tqdm(counts.columns)]
    counts.loc['n_phyla'] = [count_phyla(species_index[arg], taxonomy_map) for arg in tqdm(counts.columns)]
    counts.loc['cluster_size'] = [len(species_index[arg]) for arg in tqdm(counts.columns)]
    ...
    metadata.to_csv('arg_metadata_updated.tsv', sep = '\t', header=True, index=True)
